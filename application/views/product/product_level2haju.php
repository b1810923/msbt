      ...
      <div id="pro_img">
        <div id="pro_zoom">
          <span class='zoom' id='ex1'>
            <img id="def_img" class="small" src="<?php echo base_url();?>img/product/category/<?php echo $level2Pro['category'].'/'.ucfirst($level2Pro['level2Eng']);?>/<?php echo ucfirst($level2Pro['level2Eng']).'_'.$first.'_01'; ?>_01.jpg" />
            </span>
        </div>
        <div id="pro_thumb">
        <ul id="pro_ul">
           <?php for($i=1; $i<=$firstNum; $i++){ ?>
            <li><img id="zoom_0<?php echo $i;?>" style="cursor: pointer;" class="thumb" src="<?php echo base_url();?>img/product/category/<?php echo $level2Pro['category'].'/'.ucfirst($level2Pro['level2Eng']);?>/<?php echo ucfirst($level2Pro['level2Eng']).'_'.$first.'_01';?>_0<?php echo $i;?>.jpg" onclick="setTimeout(changePic('<?php echo $first; ?>','<?php echo $i;?>','<?php echo '01';?>') ,5000)" /></li>
           <?php } ?>  
        </ul>
        </div>
      </div>
      <div id="pro_content">
        <div id="line">
          ...
          <div id="pro_colorselect">
              ...
              <select id="select">
                  <?php foreach($level2 as $item): 
                    if($item['index']<10){$item['index']='0'.$item['index'];}
                  ?>
                  <option value="<?php echo $item['level3Eng'].'-'.$item['picNum'].'-'.$item['index'];?>"><?php echo $item['level3Name'].' '.ucfirst($item['level3Eng']);?></option>
                  <?php endforeach ?>        
              </select>
          </div>
          <div id="pro_text">
          ...
          <div id="pro_color">
            ...
            ?>
              <img class="ori" id="<?php echo $colors['level3Eng'].'-'.$colors['index'];?>" style="cursor: pointer;" title="<?php echo $colors['level3Name'];?>" alt="<?php echo $colors['level3Name'];?>" onclick="changePicList('<?php echo $colors['level3Eng'];?>','<?php echo $colors['picNum'];?>','<?php echo $colors['index'];?>');" src="<?php echo base_url();?>img/product/category/<?php echo $level2Pro['category'].'/'.ucfirst($level2Pro['level2Eng']).'/';?>color/<?php echo ucfirst($level2Pro['level2Eng']).'_Color_'.$colors['level3Eng'].'_'.$colors['index'];?>.jpg" />
            <?php endforeach ?>
          </div>
          <?php 
          $sampleKey = 'N';
...
$("#select").change(function(e){
  ...
  setTimeout(changePicList(color[0],color[1],color[2]), 5000 );
});

function changePic(picName,num, picIndex){
  var picIndexModi = picIndex; // pic index zero


  pic_url = "<?php echo base_url();?>"+'img/product/category/'+"<?php echo $category.'/'.ucfirst($level2Eng).'/'.ucfirst($level2Eng);?>"+'_'+picName+'_'+picIndexModi+'_0'+num+'.jpg';
  
  $('#def_img').attr('src', pic_url);
  $('#ex1').zoom();
}
// 換一組圖
function changePicList(picName, picNum, picIndex){
  var picIndexModi = picIndex; // pic index zero
  pic_url = "<?php echo base_url();?>"+'img/product/category/'+"<?php echo $category.'/'.ucfirst($level2Eng).'/'.ucfirst($level2Eng);?>"+'_'+picName+'_'+picIndexModi+'_01.jpg';
  ...
  for(var i=1; i<=picNum;i++){
    ...
    var imgSrc = "<?php echo base_url();?>"+'img/product/category/'+"<?php echo $category.'/'.ucfirst($level2Eng).'/'.ucfirst($level2Eng);?>"+'_'+picName+'_'+picIndexModi+'_0'+i+'.jpg';
    var newLi = $("<li></li>");
    var newImg = $("<img id=zoom_0"+i+" />").attr('class','thumb').attr('src',imgSrc).attr('style','cursor:pointer').attr('onclick',"changePic(\'"+picName+"\',\'"+i+"\',\'"+picIndex+"\')");
  ...
  var selectVal = picName+'-'+picNum+'-'+picIndex;
