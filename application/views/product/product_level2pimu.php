...
foreach($level2 as $item): 
  $level2Eng =  $item['level2Eng']; // wooden/highGlossed/fauxWood/westernCedar/bamboo
  $level2id = $item['level2Id'];
  $category = $item['category']; 
  $first = $item['level3Name']; // 001
...
<div id="content">
  <div id="topMenu">
    ...
    <div id="page_pro">
      <div id="pro_img">
        <div id="pro_zoom">
          <span class='zoom' id='ex1'>
            <img id="def_img" class="small" src="<?php echo base_url();?>img/product/category/<?php echo $level2Pro['category'];?>/Pimu/<?php echo ucfirst($level2Pro['level2Eng']);?>/<?php echo ucfirst($level2Pro['level2Eng']).'_'.$first; ?>_01.jpg" />
            </span>
        </div>
        <div id="pro_thumb">
        <ul id="pro_ul">
           <?php for($i=1; $i<=$firstNum; $i++){ ?>
            <li><img id="zoom_0<?php echo $i;?>" style="cursor: pointer;" class="thumb" src="<?php echo base_url();?>img/product/category/<?php echo $level2Pro['category'];?>/Pimu/<?php echo ucfirst($level2Pro['level2Eng']).'/'.ucfirst($level2Pro['level2Eng']).'_'.$first;?>_0<?php echo $i;?>.jpg" onclick="setTimeout(changePic('<?php echo $first; ?>','<?php echo $i;?>') ,5000)" /></li>
           <?php } ?>  
        </ul>
        </div>
      </div>
      <div id="pro_content">
        <div id="line">
          <h3><?php echo 'Pimu 實木百葉簾 – '.$level2Pro['level2Name'];?></h3>
          <div class="pro_subheading"><?php echo $level2Pro['title'];?></div>
          <div id="pro_colorselect">
              顏色：
              <select id="select">
                  <?php foreach($level2 as $item): ?>
                  <option value="<?php echo $item['level3Name'].'-'.$item['picNum'];?>"><?php echo $item['level3Name'];?></option>
                  <?php endforeach ?>        
              </select>
          </div>
          <div id="pro_text">
          ...
          <div id="pro_color">
            ...
            ?>
              <img class="ori" id="<?php echo $colors['level3Name'];?>" style="cursor: pointer;" title="<?php echo $colors['level3Name'];?>" alt="<?php echo $colors['level3Name'];?>" onclick="changePicList('<?php echo $colors['level3Name'];?>','<?php echo $colors['picNum'];?>');" src="<?php echo base_url();?>img/product/category/<?php echo $level2Pro['category'].'/Pimu/'.ucfirst($level2Pro['level2Eng']).'/color/'.ucfirst($level2Pro['level2Eng']).'_Color_'.$colors['level3Name'].'_'.$colors['index'];?>.jpg" />
            <?php endforeach ?>
          </div>
          <?php 
          $sampleKey = 'N';
...
function changePic(picName,num){
  pic_url = "<?php echo base_url();?>"+'img/product/category/'+"<?php echo $category.'/Pimu/'.ucfirst($level2Eng).'/'.ucfirst($level2Eng);?>"+'_'+picName+'_0'+num+'.jpg';
...
function changePicList(picName, picNum){
  pic_url = "<?php echo base_url();?>"+'img/product/category/'+"<?php echo $category.'/Pimu/'.ucfirst($level2Eng).'/'.ucfirst($level2Eng);?>"+'_'+picName+'_01.jpg';
  ...
  for(var i=1; i<=picNum;i++){
    ...
    var imgSrc = "<?php echo base_url();?>"+'img/product/category/'+"<?php echo $category.'/Pimu/'.ucfirst($level2Eng).'/'.ucfirst($level2Eng);?>"+'_'+picName+'_0'+i+'.jpg';
    var newLi = $("<li></li>");
