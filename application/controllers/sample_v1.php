...
//					'Linen Black'	=>	'桔梗 亞麻黑',
	...
	function samples_gets(){
		...
		$tel_areacode	=	$this->input->post('tel_zipcode',true);
	...
	//增加簽名檔
		...
		$sign .= '<font size=1 color=#666666>幔室布緹有限公司<br/>Maison Boutique Co., Ltd.</font><br/>';
		$sign .= '<br/><font size=1 color=#666666>■ 電話：(03)488-0250<br/>■ 傳真：(03)485-3023<br/>■ 官方網站：www.msbt.com.tw<br/>■ 客服信箱：service@msbt.com.tw</font>';
...
if(!$mail->Send()) {
  echo "<script>alert('Sorry, one occur the error. Please send mail to MSBT service, thank you.')</script>";
} else {
...
//	$this->load->database($dsn);
	...
	function check_sample_captcha(){
		echo $this->check_captcha();
	}
	...
	function check_captcha(){
		...
		if(($input_captcha == null) || ($input_captcha =='') || strlen($input_captcha)==0){
			...
			if($query->num_rows() == 0){
				...
				//CHECK SAMPLE NUM
				$check_samples = $this->input->post('SamplesValue', true);

				if( ($check_samples == '無') || ($check_samples == '') ){
					return 'no';
				}else{
					return 'match';
				}
